//! Delta Lake integration for Aqueducts
//!
//! This crate provides Delta Lake table functionality for the Aqueducts framework.
//! It includes:
//!
//! - DeltaDestination for writing data to Delta Lake tables
//! - Registration and write operations for Delta Lake tables
//! - Support for different write modes (append, upsert, replace)
...
pub async fn create(table_def: &DeltaDestination) -> Result<DeltaTable> {
...
#[instrument(skip_all, err)]
pub async fn write(table_def: &DeltaDestination, data: DataFrame) -> Result<DeltaTable> {
...
/// Register a Delta table with a SessionContext
#[instrument(skip_all, err)]
pub async fn register_destination(
    ctx: Arc<SessionContext>,
    table_def: &DeltaDestination,
) -> Result<()> {
    let _ = create(table_def).await?;
    register_object_store(ctx, &table_def.location, &table_def.storage_options)
        .map_err(Error::from)?;
    Ok(())
}
